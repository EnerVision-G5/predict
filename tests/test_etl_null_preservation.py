...
Ce fichier est le test d'acceptation du ticket. Il ne teste aucun étage en
particulier : il rejoue une panne de capteur de bout en bout, de la mesure
...
milieu d'une série est reconstruite et reste en base avec sa valeur brute
nulle. Une coupure en fin de série est reportée depuis la dernière valeur
connue. Une coupure sans passé exploitable n'est pas inventée : la mesure est
chargée brute et rangée dans `mesure_exclu` avec sa cause.
"""
...
from etl.quality import QUALITY_CRITICAL
...
def ingest(readings):
    """Rejoue la chaîne d'ingestion complète sur un lot de mesures."""
    return impute_frame(deduplicate(to_frame(readings)))


def test_an_outage_inside_a_series_keeps_its_raw_null(make_reading) -> None:
    frame = ingest(
        [
...
def test_an_outage_inside_a_series_is_not_excluded(make_reading) -> None:
...
def test_an_outage_without_a_past_is_filed_with_its_cause(
    ...
    frame = ingest([outage(make_reading, 8)])
...
def test_an_unfiled_outage_still_reaches_the_base_raw(make_reading) -> None:
    # L'exclusion range la mesure, elle ne la remplace pas : la ligne brute
    # est chargée dans `mesure` avec sa cause, panne comprise.
    frame = ingest([outage(make_reading, 8)])
...
def test_a_prolonged_outage_keeps_every_raw_null(make_reading) -> None:
    frame = ingest(
        [
            healthy(make_reading, 8, 100.0),
...
def test_a_healthy_series_carries_no_trace_of_imputation(make_reading) -> None:
    frame = ingest([healthy(make_reading, hour, 100.0) for hour in (8, 9)])
