...
n'est pas inventée : elle reste nulle, et `etl.exclude` rangera la mesure.
...
# Valeurs admises par la contrainte CHECK de la colonne imputation_method.
IMPUTATION_METHODS = (METHOD_NONE, METHOD_LOCF, METHOD_INTERPOLATION)

SOURCE_COLUMN = "consumption_kw"
...
def _candidates(
    ...
    dated = frame[frame["ts"].notna()]
    for _, group in dated.groupby("site_id", sort=False):
